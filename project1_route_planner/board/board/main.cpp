...
// Directional deltas in the form of a 2D array.
// An array is a C++ container much like a vector, although without the ability to change size after initialization. Arrays can be accessed and iterated over just as vectors.
// Iterate over these delta values to check the neighbors in each direction:
 ...
 * ExpandNeighbors function that accepts references to the following: The current node, the open vector, the grid, and an int array for the goal coordinates.
 */
void ExpandNeighbors(const vector<int>& current_node, int goal_point[2], vector<vector<int>>& open, vector<vector<State>>& grid)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (CheckValidCell(x2, y2, grid))
		{
			...
			int g2 = g + 1;
			int h2 = Heuristic(x2, y2, goal_point[0], goal_point[1]);
